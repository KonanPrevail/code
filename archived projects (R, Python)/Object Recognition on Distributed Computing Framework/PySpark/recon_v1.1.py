...
imagerddu = sc.emptyRDD()

# with the reconstruction distance changes, the shift in px between 2D images are from 78px-64px (15 reconstructed distances)
# append all these images to imagerddu RDD
# if an image name is car_1_angle_1_0003.jpg
# imagerddu will have (key,value) -> (car_1_angle_1,(Imagearray,3,78))
# 78px - 64px are the shifts in px corresponding to the different reconstruction distances
# imagerddu will now have 3(cars)*6(angles)*28(images)*15(reconstruction distances)=7560
for shift_px in range(78,63,-1):	
    imagerddu = imagerddu.union(images.map(lambda (x,y): (findKey(x,shift_px),(image_array,findNumber(x),shift_px))))
