...
nnr = MLPRegressor(solver='adam', activation='relu', alpha=10, hidden_layer_sizes=(1000,100), random_state=1)
nnr = nnr.fit(x_train, y_train)

# Predict training and test data
trainpred = nnr.predict(x_train)
testpred = nnr.predict(x_test)
...
#==============================================================================
score = nnr.score(x_train, y_train)
...
plt.scatter(x_train[:,0], y_train, color='darkorange', label='Training data', s=0.1)
plt.hold('on')
plt.scatter(x_train[:,0], trainpred, color='navy', label='NN model', s=0.1)
plt.xlabel('data')
plt.ylabel('target')
