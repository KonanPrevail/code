...
for i in range(x.shape[1]):
    plt.hist(xTrain.iloc[:,i])

# Create and train a decision tree classifier, assume Gaussian
gnb = GaussianNB()
gnb = gnb.fit(xTrain, yTrain)

# Classify training and test data
trainpred = gnb.predict(xTrain)
testpred = gnb.predict(xTest)
...
gnb = GaussianNB()
gnb = gnb.fit(xTrain[['SALES','AGE']], yTrain)
...
plt.title("Decision surface of a naive bayes using paired features")
plt.legend()
plt.show()