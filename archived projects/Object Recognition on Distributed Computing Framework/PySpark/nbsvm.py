...
# Car 1 will be considered as o and 2,3 as 1 for svm analysis
def classify1(name):
    if(name[4]==1):
        imgId = 0
    else:
...
imageVec.persist()
#imageNB = image1D.map(lambda (x,y):(x[4],y)).map(lambda (x,y): LabeledPoint(x, Vectors.dense((y))))

imageSVM1 = imageVec.map(lambda (x,y):(classify1(x),y)).map(lambda (x,y): LabeledPoint(x, Vectors.dense((y))))

imageSVM23 = imageVec.filter(lambda (x,y): x[4]!="1").map(lambda (x,y):(classify23(x),y)).map(lambda (x,y): LabeledPoint(x, Vectors.dense((y))))

training1, test1 = imageSVM1.randomSplit([0.5, 0.5], seed=0)
training23, test23 = imageSVM23.randomSplit([0.5, 0.5], seed=0)

# Train SVM model.
#model = NaiveBayes.train(training, 1.0)
model1 = SVMWithSGD.train(training1, iterations=3)
model23 = SVMWithSGD.train(training23, iterations=3)
# Make prediction and test accuracy.
#predictionAndLabel1 = test1.map(lambda p: (model1.predict(p.features), p.label))
prediction1 = test1.map(lambda p: (model1.predict(p.features), p.features))
label23 = prediction1.filter(lambda (x,y): x==1)

predictionAndLabel23 = label23.map(lambda (x,y): (model23.predict(y), y))

predictlabel23 = predictionAndLabel23.map(lambda (x,y): (x+2,y))

predictlabel1 = prediction1.filter(lambda (x,y): x!=1).map(lambda (x,y): (1,y))
predictedfull = predictlabel1.union(predictlabel23)

#svmrev = image1D.map(lambda (x,y): LabeledPoint(x, Vectors.dense((y))))
#svmrev = image1D.map(lambda (x,y): (Vectors.dense(y), x))
#predictedrev = predictedfull.map(lambda (x,y): (y,x))
#testsvm = svmrev.join(predictedrev)

prediction1.saveAsTextFile("/Users/apple/Documents/uconn/PatternRecognitionSpark/SVM1")
predictionAndLabel23.saveAsTextFile("/Users/apple/Documents/uconn/PatternRecognitionSpark/SVM23")

afterexec = time.time()
print "total time"
print str(afterexec - beforeexec)
